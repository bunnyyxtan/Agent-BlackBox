...
export function StorageJobCard({
  ...
  return (
    <GlassCard className="p-5 group transition-all duration-300 hover:border-indigo-500/30 hover:bg-white/[0.04]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <ProtocolLogo protocol="walrus" size="lg" />
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-white tracking-tight">{job.fileName}</p>
            <div className="flex items-center gap-2 mt-0.5">
              <p className="truncate font-mono text-[0.65rem] text-zinc-500">{job.uploadJobId}</p>
            </div>
          </div>
        </div>
      ...
      <dl className="mt-5 grid grid-cols-2 gap-4 border-y border-white/[0.05] py-4 text-xs bg-black/10 -mx-5 px-5">
        <div>
          <dt className="text-zinc-600 uppercase tracking-widest font-mono text-[0.6rem] mb-1">Provider</dt>
          <dd className="flex min-w-0 items-center gap-2 truncate text-zinc-300 font-medium">
        ...
        <div>
          ...
          <dd className="truncate text-zinc-300 font-medium">
        ...
        <div>
          ...
          <dd className="truncate text-zinc-300 font-medium">{job.fileType ?? "application/json"}</dd>
        </div>
        <div>
      ...
      <div className="mt-4 space-y-2">
        <p className="truncate font-mono text-[0.65rem] text-indigo-400/70 bg-indigo-500/10 px-2 py-1 rounded border border-indigo-500/20 inline-block">
          Blob: {shortHash(job.blobId, 10, 8)}
        </p>
        <p className="truncate font-mono text-[0.65rem] text-zinc-500" title={job.blobObjectId || "Not available"}>
          Object: {job.blobObjectId ? shortHash(job.blobObjectId, 10, 8) : "Not available"}
        </p>
      </div>
      
      <div className="mt-5 flex flex-wrap gap-2">
        <button type="button" onClick={viewStatus} disabled={Boolean(action)} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-white transition hover:border-indigo-400/40 hover:bg-indigo-400/10 disabled:cursor-wait disabled:opacity-55">
          <iconify-icon icon="solar:eye-line-duotone" />
          {action === "status" ? "Checking..." : "View Status"}
        </button>
        <CopyButton value={job.uploadJobId} label="Copy ID" />
        <button type="button" onClick={directRead} disabled={Boolean(action)} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-white transition hover:border-indigo-400/40 hover:bg-indigo-400/10 disabled:cursor-wait disabled:opacity-55">
          <iconify-icon icon="solar:download-minimalistic-line-duotone" />
          {action === "read" ? "Reading..." : "Direct Read"}
        </button>
        <button type="button" onClick={verifyBlob} disabled={Boolean(action)} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-white transition hover:border-indigo-400/40 hover:bg-indigo-400/10 disabled:cursor-wait disabled:opacity-55">
          <iconify-icon icon="solar:shield-check-line-duotone" />
          {action === "verify" ? "Verifying..." : "Verify Blob"}
        </button>
        <button type="button" disabled title="Direct Walrus renewal controls require a later signed lifecycle flow." className="flex cursor-not-allowed items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-white opacity-55">
